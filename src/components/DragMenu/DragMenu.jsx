        ...
        <div className={dragMenu ? "open-drag-menu" : "drag-menu"}>
            <div className="drag-menu-content container p-5">
                ...
                <form onSubmit={(e) => handleSubmit(e)} className="search-bar">
                    <input value={searchTerm} onChange={(e) => handleInputChange(e.target.value.toLocaleLowerCase())} type="text" placeholder='Axtarış...' name="search" className='ps-2 pe-4' />
                    <button type="submit"><BiSearchAlt /></button>
                </form>
                <ul className='p-0 mt-2'>
                    {hashtags.map((hashtag) => (
                        <li key={hashtag} className='pt-2'><Link to={`/search?searchItem=${hashtag}`} onClick={() => closeDragMenu()}>#{hashtag}</Link></li>
                    ))}
                </ul>
            </div>
        </div>
